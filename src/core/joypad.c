...
static inline GB_BOOL GB_is_directional(const struct GB_Core* gb) {
    return !!(IO_JYP & 0x10);
}

static inline GB_BOOL GB_is_button(const struct GB_Core* gb) {
    return !!(IO_JYP & 0x20);
}

static inline GB_U8 GB_joypad_get_internal(const struct GB_Core* gb) {
