...
extern unzFile ZEXPORT unzOpen2(const char *path, const zlib_filefunc_def *pzlib_filefunc32_def)
{
    ...
    uint16_t extrafield_size, char *comment, uint16_t comment_size)
{
    ...
    if (err == UNZ_OK)
    {
        ...
        {
            printf("wrong magic!!! 0x%X wanted: 0x%X\n", magic, CENTRALHEADERMAGIC);
            err = UNZ_BADZIPFILE;
        }
    }
