...
struct SafeString util_create_save_path(const char* str) {
    #ifdef EMSCRIPTEN
        struct SafeString ss = { .str = "/saves/" };
        strncat(ss.str, str, sizeof(ss.str) - strlen(ss.str));
        return util_append_extension(
            ss.str, ".sav", ExtensionOffsetType_LAST
        );
    #endif
    return util_append_extension(
        str, ".sav", ExtensionOffsetType_LAST
    );
}

struct SafeString util_create_rtc_path(const char* str) {
    #ifdef EMSCRIPTEN
        struct SafeString ss = { .str = "/rtc/" };
        strncat(ss.str, str, sizeof(ss.str) - strlen(ss.str));
        return util_append_extension(
            ss.str, ".rtc", ExtensionOffsetType_LAST
        );
    #endif
    return util_append_extension(
        str, ".rtc", ExtensionOffsetType_LAST
    );
}

struct SafeString util_create_state_path(const char* str) {
    #ifdef EMSCRIPTEN
        struct SafeString ss = { .str = "/states/" };
        strncat(ss.str, str, sizeof(ss.str) - strlen(ss.str));
        return util_append_extension(
            ss.str, ".state", ExtensionOffsetType_LAST
        );
    #endif
    return util_append_extension(
        str, ".state", ExtensionOffsetType_LAST
    );
}
