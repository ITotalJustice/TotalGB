...
static void TableSettingsHandler_WriteAll(ImGuiContext* ctx, ImGuiSettingsHandler* handler, ImGuiTextBuffer* buf)
{
    ...
    for (ImGuiTableSettings* settings = g.SettingsTables.begin(); settings != NULL; settings = g.SettingsTables.next_chunk(settings))
    {
        ...
        for (int column_n = 0; column_n < settings->ColumnsCount; column_n++, column++)
        {
            ...
            if (save_size && column->IsStretch)         buf->appendf(" Weight=%.4f", (double)column->WidthOrWeight);
...
void ImGui::DebugNodeTable(ImGuiTable* table)
{
    ...
    BulletText("OuterRect: Pos: (%.1f,%.1f) Size: (%.1f,%.1f) Sizing: '%s'", (double)table->OuterRect.Min.x, (double)table->OuterRect.Min.y, (double)table->OuterRect.GetWidth(), (double)table->OuterRect.GetHeight(), DebugNodeTableGetSizingPolicyDesc(table->Flags));
    BulletText("ColumnsGivenWidth: %.1f, ColumnsAutoFitWidth: %.1f, InnerWidth: %.1f%s", (double)table->ColumnsGivenWidth, (double)table->ColumnsAutoFitWidth, (double)table->InnerWidth, table->InnerWidth == 0.0f ? " (auto)" : "");
    BulletText("CellPaddingX: %.1f, CellSpacingX: %.1f/%.1f, OuterPaddingX: %.1f", (double)table->CellPaddingX, (double)table->CellSpacingX1, (double)table->CellSpacingX2, (double)table->OuterPaddingX);
    ...
    for (int n = 0; n < table->ColumnsCount; n++)
    {
        ...
        ImFormatString(buf, IM_ARRAYSIZE(buf),
            ...
            n, column->DisplayOrder, name, (double)(column->MinX - table->WorkRect.Min.x), (double)(column->MaxX - table->WorkRect.Min.x), (n < table->FreezeColumnsRequest) ? " (Frozen)" : "",
            column->IsEnabled, column->IsVisibleX, column->IsVisibleY, column->IsRequestOutput, column->IsSkipItems, column->DrawChannelFrozen, column->DrawChannelUnfrozen,
            (double)column->WidthGiven, (double)column->WidthRequest, (double)column->WidthAuto, (double)column->StretchWeight, (double)(column->StretchWeight > 0.0f ? (column->StretchWeight / sum_weights) * 100.0f : 0.0f),
            (double)column->MinX, (double)column->MaxX, (double)(column->MaxX - column->MinX), (double)column->ClipRect.Min.x, (double)column->ClipRect.Max.x, (double)(column->ClipRect.Max.x - column->ClipRect.Min.x),
            (double)(column->ContentMaxXFrozen - column->WorkMinX), (double)(column->ContentMaxXUnfrozen - column->WorkMinX), (double)(column->ContentMaxXHeadersUsed - column->WorkMinX), (double)(column->ContentMaxXHeadersIdeal - column->WorkMinX),
...
void ImGui::DebugNodeTableSettings(ImGuiTableSettings* settings)
{
    ...
    for (int n = 0; n < settings->ColumnsCount; n++)
    {
        ...
        BulletText("Column %d Order %d SortOrder %d %s Vis %d %s %7.3f UserID 0x%08X",
            ...
            column_settings->IsEnabled, column_settings->IsStretch ? "Weight" : "Width ", (double)column_settings->WidthOrWeight, column_settings->UserID);
    }
    TreePop();
}
