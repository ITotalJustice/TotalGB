...
void ImGui::ProgressBar(float fraction, const ImVec2& size_arg, const char* overlay)
{
    ...
    if (!overlay)
    {
        ImFormatString(overlay_buf, IM_ARRAYSIZE(overlay_buf), "%.0f%%", (double)(fraction * 100.f + 0.01f));
        overlay = overlay_buf;
    }
...
int ImGui::DataTypeFormatString(char* buf, int buf_size, ImGuiDataType data_type, const void* p_data, const char* format)
{
    ...
    if (data_type == ImGuiDataType_Float)
        return ImFormatString(buf, buf_size, format, (double)*(const float*)p_data);
...
bool ImGui::DataTypeApplyOpFromText(const char* buf, const char* initial_value_buf, ImGuiDataType data_type, void* p_data, const char* format)
{
    ...
    else if (data_type == ImGuiDataType_Double)
    {
        ...
        else if (op == '/') { if (arg1f != 0.0) *v = arg0f / arg1f; } // Divide
        else                { *v = arg1f; }                            // Assign constant
    }
...
bool ImGui::DragBehaviorT(ImGuiDataType data_type, TYPE* v, float v_speed, const TYPE v_min, const TYPE v_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    if (is_logarithmic)
    {
        ...
        g.DragCurrentAccum -= (float)(v_new_parametric - (float)v_old_ref_for_accum_remainder);
    }
    else
    {
...
void ImGui::ColorTooltip(const char* text, const float* col, ImGuiColorEditFlags flags)
{
    ...
    {
        if (flags & ImGuiColorEditFlags_NoAlpha)
            Text("#%02X%02X%02X\nR: %d, G: %d, B: %d\n(%.3f, %.3f, %.3f)", cr, cg, cb, cr, cg, cb, (double)col[0], (double)col[1], (double)col[2]);
        else
            Text("#%02X%02X%02X%02X\nR:%d, G:%d, B:%d, A:%d\n(%.3f, %.3f, %.3f, %.3f)", cr, cg, cb, ca, cr, cg, cb, ca, (double)col[0], (double)col[1], (double)col[2], (double)col[3]);
    }
    else if (flags & ImGuiColorEditFlags_InputHSV)
    {
        if (flags & ImGuiColorEditFlags_NoAlpha)
            Text("H: %.3f, S: %.3f, V: %.3f", (double)col[0], (double)col[1], (double)col[2]);
        else
            Text("H: %.3f, S: %.3f, V: %.3f, A: %.3f", (double)col[0], (double)col[1], (double)col[2], (double)col[3]);
    }
    EndTooltip();
}
...
void ImGui::ColorEditOptionsPopup(const float* col, ImGuiColorEditFlags flags)
{
    ...
    if (BeginPopup("Copy"))
    {
        int cr = IM_F32_TO_INT8_SAT(col[0]), cg = IM_F32_TO_INT8_SAT(col[1]), cb = IM_F32_TO_INT8_SAT(col[2]), ca = (flags & ImGuiColorEditFlags_NoAlpha) ? 255 : IM_F32_TO_INT8_SAT(col[3]);
        char buf[64];
        ImFormatString(buf, IM_ARRAYSIZE(buf), "(%.3ff, %.3ff, %.3ff, %.3ff)", (double)col[0], (double)col[1], (double)col[2], (double)((flags & ImGuiColorEditFlags_NoAlpha) ? 1.0f : col[3]));
        if (Selectable(buf))
    ...
    if (values_count >= values_count_min)
    {
        ...
        if (hovered && inner_bb.Contains(g.IO.MousePos))
        {
            ...
            if (plot_type == ImGuiPlotType_Lines)
                SetTooltip("%d: %8.4g\n%d: %8.4g", v_idx, (double)v0, v_idx + 1, (double)v1);
            else if (plot_type == ImGuiPlotType_Histogram)
                SetTooltip("%d: %8.4g", v_idx, (double)v0);
            idx_hovered = v_idx;
        }
...
void ImGui::Value(const char* prefix, float v, const char* float_format)
{
    if (float_format)
    {
        ...
        Text(fmt, prefix, (double)v);
    }
    else
    {
        Text("%s: %.3f", prefix, (double)v);
    }
}
