...
void mbc5_write(struct GB_Core* gb, uint16_t addr, uint8_t value)
{
    switch ((addr >> 12) & 0xF)
    {
    // RAM
...
struct MBC_RomBankInfo mbc5_get_rom_bank(struct GB_Core* gb, uint8_t bank)
{
...
struct MBC_RamBankInfo mbc5_get_ram_bank(struct GB_Core* gb)
{
    if (!(gb->cart.flags & MBC_FLAGS_RAM) || !gb->cart.ram_enabled)
    {
